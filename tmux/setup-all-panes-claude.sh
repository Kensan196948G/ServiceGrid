...
for i in "${!PANE_INDEXES[@]}"; do
    ...
    case $i in
        0) FEATURE_NAME="Feature-A (統合リーダー)" ;;
        1) FEATURE_NAME="Feature-B (UI/テスト)" ;;
        2) FEATURE_NAME="Feature-C (API開発)" ;;
        3) FEATURE_NAME="Feature-D (PowerShell)" ;;
        4) FEATURE_NAME="Feature-E (非機能要件)" ;;
...
for i in "${!PANE_INDEXES[@]}"; do
    PANE_NUM="${PANE_INDEXES[$i]}"
    case $i in
        0) echo "  ペイン$PANE_NUM: 🎯 Feature-A (統合リーダー) - プロジェクト全体管理" ;;
        1) echo "  ペイン$PANE_NUM: 🎨 Feature-B (UI/テスト) - フロントエンド開発" ;;
        2) echo "  ペイン$PANE_NUM: 🔧 Feature-C (API開発) - バックエンドAPI" ;;
        3) echo "  ペイン$PANE_NUM: 💻 Feature-D (PowerShell) - Windows環境対応" ;;
        4) echo "  ペイン$PANE_NUM: 🔒 Feature-E (非機能要件) - セキュリティ・監視" ;;
    esac
done
...
echo ""
echo "⌨️ ペイン切り替え:"
echo "  Ctrl+b + ${PANE_INDEXES[0]}: 🎯 Feature-A | Ctrl+b + ${PANE_INDEXES[1]}: 🎨 Feature-B"
echo "  Ctrl+b + ${PANE_INDEXES[2]}: 🔧 Feature-C | Ctrl+b + ${PANE_INDEXES[3]}: 💻 Feature-D"
