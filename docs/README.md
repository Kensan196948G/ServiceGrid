...
| [08_実装状況レポート.md](08_実装状況レポート.md) | プロジェクト進捗・実装状況・技術課題 | マネージャー・開発者 |
| [09_自動運用監視システム仕様書.md](09_自動運用監視システム仕様書.md) | 自動監視・エラー検出・修復システム仕様 | 運用担当者・SRE |
...
### フロントエンド
- **React 19.1.0** + **TypeScript 5.7.2** - モダンUI開発
- **Vite 6.3.5** - 高速開発・ビルド環境
- **React Router v7.6.1** - SPAルーティング（Hash Router）
- **Tailwind CSS** - ユーティリティファーストスタイリング（CDN）
- **Recharts** - データ可視化
- **Jest + Testing Library** - 包括的テスト
- **Google Gemini AI** - AI統合機能

### バックエンド
- **Node.js 22.16.0** + **Express** - REST APIサーバー
- **SQLite** - 軽量データベース管理
- **JWT** - 認証トークン管理
- **bcrypt** - パスワードハッシュ化
- **Helmet** - セキュリティヘッダー
- **自動監視ループ** - エラー検出・自動修復システム
...
### 最新追加機能（2025年6月9日）
- ✅ **自動運用監視システム** - 30秒間隔のヘルスチェック・自動修復
- ✅ **React 19.1.0対応** - 最新機能活用・パフォーマンス向上
- ✅ **Node.js 22.16.0対応** - 最新ランタイム・セキュリティ強化
- ✅ **エラー自動修復** - 依存関係・プロセス・データベース自動修復
