...
import { itemStatusToJapanese, releaseTypeToJapanese, releaseTypeToIcon } from '../localization'; 
...
const ReleaseManagementPage: React.FC = () => {
  ...
  const servicesAffectedOptions = ['顧客ポータル', '社内基幹システム', 'モバイルアプリ', 'APIゲートウェイ', '認証サーバー', '財務システム', '人事システム', 'その他'];
  ...
  const listColumns: Array<{ Header: string; accessor: keyof Release | ((row: Release) => ReactNode) }> = [
    ...
    { Header: '影響サービス', accessor: (row) => row.servicesAffected.join(', ').substring(0,30) + (row.servicesAffected.join(', ').length > 30 ? '...' : '')},
      ...
      {/* Filters for List */}
       <Card title="リリース計画フィルター">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 p-4 items-end">
          ...
          <Input label="影響サービス" placeholder="サービス名..." value={serviceFilter} onChange={e => {setServiceFilter(e.target.value); setCurrentPage(1);}} />
      ...
      {editingRelease && (
        <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={editingRelease.id ? 'リリース計画編集' : '新規リリース計画登録'} size="xl">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">影響を受けるサービス (複数選択可)</label>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-40 overflow-y-auto border p-2 rounded">
                {servicesAffectedOptions.map(service => (
                  <label key={service} className="flex items-center space-x-2 text-sm">
                    <input
                      type="checkbox"
                      value={service}
                      checked={editingRelease.servicesAffected?.includes(service)}
                      onChange={(e) => {
                        const checked = e.target.checked;
                        const currentServices = editingRelease.servicesAffected || [];
                        if (checked) {
                          handleServicesAffectedChange([...currentServices, service]);
                        } else {
                          handleServicesAffectedChange(currentServices.filter(s => s !== service));
                        }
                      }}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Input label="テストリード (任意)" name="testLead" value={editingRelease.testLead || ''} onChange={handleInputChange} />
                <Input label="展開リード (担当者)" name="deploymentLead" value={editingRelease.deploymentLead || ''} onChange={handleInputChange} />
            </div>
