...
import { getIncidents, addIncident, updateIncident, deleteIncident } from '../services/mockItsmService';
...
const IncidentPage: React.FC = () => {
  ...
  const fetchIncidents = useCallback(async () => {
    ...
    try {
      ...
      setNotification({ message: 'インシデントの読み込みに失敗しました。', type: NotificationType.ERROR });
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingIncident.id) {
        await updateIncident(editingIncident.id, { ...editingIncident, reportedBy: editingIncident.reportedBy || user.username } as Incident);
        setNotification({ message: 'インシデントが正常に更新されました。', type: NotificationType.SUCCESS });
      } else {
        await addIncident({ ...editingIncident, reportedBy: user.username } as Omit<Incident, 'id' | 'createdAt' | 'updatedAt'>);
      ...
      setNotification({ message: 'インシデントの保存に失敗しました。', type: NotificationType.ERROR });
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('このインシデントを削除してもよろしいですか？')) {
      try {
        ...
        setNotification({ message: 'インシデントの削除に失敗しました。', type: NotificationType.ERROR });
      }
    }
  };
